...
"""
Prompt-Tuning SFT (Hugging Face) for CA prompts — no LoRA.

This trains a small set of virtual prompt tokens (soft prompt) using PEFT's
PromptTuning on a HF CausalLM (default: Qwen/Qwen2.5-0.5B-Instruct). Base
weights stay frozen. We supervise exact TABLE/RULE/Y strings for ECA rules.

Usage (CPU is fine, smaller models recommended):

  python scripts/sft_prompt_hf.py \
    --model-id Qwen/Qwen2.5-0.5B-Instruct \
    --steps 2000 --lr 2e-4 --batch 4 \
    --rules 30,90,110 --tasks table,rule \
    --width 32 --pairs 8 --rollout-min 3 --rollout-max 10 \
    --n-virtual 20 \
    --out-dir outputs/ca_prompt

Evaluation: the script saves eval_table.json with predicted TABLE digits.
You can also load the PEFT adapter in HF and generate. Converting to MLX
requires keeping the prompt tuner in HF or prepending learned virtual tokens in
text form (not recommended). For MLX inference, prefer SFT that modifies base
weights or LoRA merge; for strict "no LoRA", use HF at inference time.
"""
...
def main():
    ...
    ap.add_argument("--out-dir", type=str, default="outputs/ca_prompt")
    ...
    # Prompt tuning via PEFT (no LoRA)
    try:
    ...
    pt_cfg = PromptTuningConfig(
        task_type="CAUSAL_LM",
        num_virtual_tokens=args.n_virtual,
